...
func (c *client) handleIETFQUICPacket(p *receivedPacket) error {
	...
	if p.header.IsLongHeader {
		if p.header.Type != protocol.PacketTypeRetry && p.header.Type != protocol.PacketTypeHandshake {
			return fmt.Errorf("Received unsupported packet type: %s", p.header.Type)
		}
		c.logger.Debugf("len(packet data): %d, payloadLen: %d", len(p.data), p.header.PayloadLen)
